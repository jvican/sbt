...
trait Init[Scope] {
  ...
  def setting[T](key: ScopedKey[T],
    init: Initialize[T],
    pos: SourcePosition = NoPosition): Setting[T] =
    new Setting[T](key, init, pos)
  ...
  def app[K[L[x]], T](inputs: K[Initialize])(f: K[Id] => T)(
    implicit alist: AList[K]): Initialize[T] =
    new Apply[K, T](f, inputs, alist)
  def uniform[S, T](inputs: Seq[Initialize[S]])(
    f: Seq[S] => T): Initialize[T] =
    new Apply[({ type l[L[x]] = List[L[S]] })#l, T](f,
      inputs.toList,
      AList.seq[S])
  ...
  @deprecated("Use the version with default arguments and default parameter.",
    "0.13.7")
  final def derive[T](s: Setting[T],
    allowDynamic: Boolean,
    filter: Scope => Boolean,
    trigger: AttributeKey[_] => Boolean): Setting[T] =
  ...
  def deriveAllowed[T](s: Setting[T], allowDynamic: Boolean): Option[String] =
    s.init match {
      case _: Bind[_, _] if !allowDynamic =>
        Some("Cannot derive from dynamic dependencies.")
      case _ => None
    }
  // id is used for equality
  private[sbt] final def defaultSetting[T](s: Setting[T]): Setting[T] =
    s.default()
  ...
  def asTransform(s: Settings[Scope]): ScopedKey ~> Id =
    new (ScopedKey ~> Id) {
  ...
  def asFunction[T](s: Settings[Scope]): ScopedKey[T] => T =
    k => getValue(s, k)
  ...
  private final class InvalidReference(val key: ScopedKey[_])
    extends RuntimeException(
      "Internal settings error: invalid reference to " + showFullKey(key))

  private[this] def applyDefaults(ss: Seq[Setting[_]]): Seq[Setting[_]] = {
    val (defaults, others) =
      Util.separate[Setting[_], DefaultSetting[_], Setting[_]](ss) {
  ...
  def addLocal(init: Seq[Setting[_]])(
    implicit scopeLocal: ScopeLocal): Seq[Setting[_]] =
  ...
  def delegate(sMap: ScopedMap)(implicit delegates: Scope => Seq[Scope],
    ...
    if (undefineds.isEmpty)
      ...
      throw Uninitialized(sMap.keys.toSeq,
        delegates,
        undefineds.values.flatten.toList,
        false)
  }
  private[this] def delegateForKey[T](
    ...
    val definedAt =
      skeys.find(sk => (selfRefOk || ref.key != sk) && (sMap contains sk))
  ...
  final class Uninitialized(val undefined: Seq[Undefined],
    override val toString: String)
      extends Exception(toString)
  final class Undefined private[sbt] (val defining: Setting[_],
    ...
    def this(definingKey: ScopedKey[_],
      referencedKey: ScopedKey[_],
      derived: Boolean) =
      this(fakeUndefinedSetting(definingKey, derived), referencedKey)
  }
  ...
  def Undefined(definingKey: ScopedKey[_],
    referencedKey: ScopedKey[_],
    derived: Boolean): Undefined =
    new Undefined(fakeUndefinedSetting(definingKey, derived), referencedKey)
  private[this] def fakeUndefinedSetting[T](definingKey: ScopedKey[T],
    d: Boolean): Setting[T] = {
    val init: Initialize[T] = pure(
      () => sys.error("Dummy setting for compatibility only."))
  ...
  def Uninitialized(validKeys: Seq[ScopedKey[_]],
    ...
    val keysString = keys
      .map(u => showUndefined(u, validKeys, delegates))
      .mkString("\n\n  ", "\n\n  ", "")
    new Uninitialized(
      keys,
  ...
  final class Flattened(val key: ScopedKey[_],
    val dependencies: Iterable[ScopedKey[_]])
  ...
  def flattenLocals(compiled: CompiledMap): Map[ScopedKey[_], Flattened] = {
    ...
    def flatten(cmap: Map[ScopedKey[_], Flattened],
      ...
      new Flattened(
        key,
        deps.flatMap(dep =>
          if (dep.key.isLocal) cmap(dep).dependencies else dep :: Nil))
  ...
  private[this] def deriveAndLocal(init: Seq[Setting[_]])(
    ...
    final class Deriveds(val key: AttributeKey[_],
        val settings: mutable.ListBuffer[Derived]) {
    ...
    for (s <- derived) {
      ...
      derivsByDef
        .getOrElseUpdate(key, new Deriveds(key, new mutable.ListBuffer))
        .settings += s
    }
    ...
    val derivedBy =
      new mutable.HashMap[AttributeKey[_], mutable.ListBuffer[Derived]]
    ...
    def allDepsDefined(d: Derived,
      scope: Scope,
      local: Set[AttributeKey[_]]): Boolean =
      ...
      def localAndDerived(d: Derived): Seq[Setting[_]] = {
        ...
        outputScope collect {
          case s if !d.inScopes.contains(s) && d.setting.filter(s) =>
            val local =
              d.dependencies.flatMap(dep => scopeLocal(ScopedKey(s, dep)))
    ...
    allDefs flatMap {
      case d: DerivedSetting[_] =>
        (derivedToStruct get d map (_.outputs)).toStream.flatten;
      case s => Stream(s)
    }
  }
  ...
  sealed trait Initialize[T] {
    ...
    def validateReferenced(g: ValidateRef): ValidatedInit[T] =
      validateKeyReferenced(new ValidateKeyRef {
        def apply[T](key: ScopedKey[T], selfRefOk: Boolean) = g(key)
      })
    ...
    private[this] def zipTupled[S, U](o: Initialize[S])(
      f: ((T, S)) => U): Initialize[U] =
      new Apply[({ type l[L[x]] = (L[T], L[S]) })#l, U](f,
        (this, o),
        AList.tuple2[T, S])

    /** A fold on the static attributes of this and nested Initializes. */
    private[sbt] def processAttributes[S](init: S)(
      f: (S, AttributeMap) => S): S
  }
  object Initialize {
    implicit def joinInitialize[T](s: Seq[Initialize[T]]): JoinInitSeq[T] =
      new JoinInitSeq(s)
  ...
  object SettingsDefinition {
    implicit def unwrapSettingsDefinition(
      d: SettingsDefinition): Seq[Setting[_]] = d.settings
    implicit def wrapSettingsDefinition(
      ss: Seq[Setting[_]]): SettingsDefinition = new SettingList(ss)
  }
  sealed trait SettingsDefinition {
    def settings: Seq[Setting[_]]
  }
  final class SettingList(val settings: Seq[Setting[_]])
  ...
  sealed class Setting[T] private[Init] (val key: ScopedKey[T],
    ...
    def mapReferenced(g: MapScoped): Setting[T] =
      make(key, init mapReferenced g, pos)
    @deprecated("Will be made private.", "0.13.2")
    def validateReferenced(
      g: ValidateRef): Either[Seq[Undefined], Setting[T]] =
      (init validateReferenced g).right.map(newI => make(key, newI, pos))

    private[sbt] def validateKeyReferenced(
      g: ValidateKeyRef): Either[Seq[Undefined], Setting[T]] =
    ...
    def mapInit(f: (ScopedKey[T], T) => T): Setting[T] =
      make(key, init(t => f(key, t)), pos)
    @deprecated("Will be made private.", "0.13.2")
    def mapConstant(g: MapConstant): Setting[T] =
      make(key, init mapConstant g, pos)
    ...
    private[sbt] def mapInitialize(
      f: Initialize[T] => Initialize[T]): Setting[T] =
      make(key, f(init), pos)
    override def toString = "setting(" + key + ") at " + pos

    protected[this] def make[T](key: ScopedKey[T],
      init: Initialize[T],
      pos: SourcePosition): Setting[T] =
      new Setting[T](key, init, pos)
    ...
    private[sbt] def default(
      id: => Long = nextDefaultID()): DefaultSetting[T] =
  ...
  private[Init] sealed class DerivedSetting[T](
    ...
    override def make[T](key: ScopedKey[T],
      init: Initialize[T],
      pos: SourcePosition): Setting[T] =
      new DerivedSetting[T](key, init, pos, filter, trigger)
    protected[sbt] override def isDerived: Boolean = true
    override def default(_id: => Long): DefaultSetting[T] =
      new DerivedSetting[T](sk, i, p, filter, trigger) with DefaultSetting[T] {
        val id = _id
      }
  ...
  object DefaultSetting {
    def apply[T](sk: ScopedKey[T],
      i: Initialize[T],
      p: SourcePosition,
      _id: Long) = new Setting[T](sk, i, p) with DefaultSetting[T] {
      val id = _id
    }
  }

  private[this] def handleUndefined[T](vr: ValidatedInit[T]): Initialize[T] =
    vr match {
  ...
  private[this] lazy val getValidated =
    new (ValidatedInit ~> Initialize) {
      def apply[T](v: ValidatedInit[T]) = handleUndefined[T](v)
    }
  ...
  private[this] def mapReferencedT(g: MapScoped) =
    new (Initialize ~> Initialize) {
      def apply[T](i: Initialize[T]) = i mapReferenced g
    }

  private[this] def mapConstantT(g: MapConstant) =
    new (Initialize ~> Initialize) {
      def apply[T](i: Initialize[T]) = i mapConstant g
    }

  private[this] def evaluateT(g: Settings[Scope]) =
    new (Initialize ~> Id) { def apply[T](i: Initialize[T]) = i evaluate g }

  private[this] def deps(ls: Seq[Initialize[_]]): Seq[ScopedKey[_]] =
  ...
  sealed trait Keyed[S, T] extends Initialize[T] {
    ...
    final def apply[Z](g: T => Z): Initialize[Z] =
      new GetValue(scopedKey, g compose transform)
    final def evaluate(ss: Settings[Scope]): T =
      transform(getValue(ss, scopedKey))
    final def mapReferenced(g: MapScoped): Initialize[T] =
      new GetValue(g(scopedKey), transform)
    private[sbt] final def validateKeyReferenced(
      g: ValidateKeyRef): ValidatedInit[T] = g(scopedKey, false) match {
    ...
    private[sbt] def processAttributes[S](init: S)(
      f: (S, AttributeMap) => S): S = init
  }
  private[this] final class GetValue[S, T](val scopedKey: ScopedKey[S],
    val transform: S => T)
      extends Keyed[S, T]
  ...
  private[sbt] final class TransformCapture(val f: Initialize ~> Initialize)
    ...
    def apply[Z](g2: (Initialize ~> Initialize) => Z): Initialize[Z] =
      map(this)(g2)
    def evaluate(ss: Settings[Scope]): Initialize ~> Initialize = f
    def mapReferenced(g: MapScoped) =
      new TransformCapture(mapReferencedT(g) ∙ f)
  ...
  private[sbt] final class ValidationCapture[T](val key: ScopedKey[T],
    val selfRefOk: Boolean)
  ...
  private[sbt] final class Bind[S, T](val f: S => Initialize[T],
    val in: Initialize[S])
    ...
    def mapReferenced(g: MapScoped) =
      new Bind[S, T](s => f(s) mapReferenced g, in mapReferenced g)
    def validateKeyReferenced(g: ValidateKeyRef) =
      (in validateKeyReferenced g).right.map { validIn =>
        new Bind[S, T](s => handleUndefined(f(s) validateKeyReferenced g),
          validIn)
      }
    def mapConstant(g: MapConstant) =
      new Bind[S, T](s => f(s) mapConstant g, in mapConstant g)
    private[sbt] def processAttributes[S](init: S)(
      f: (S, AttributeMap) => S): S = in.processAttributes(init)(f)
  }
  private[sbt] final class Optional[S, T](val a: Option[Initialize[S]],
    val f: Option[S] => T)
      extends Initialize[T] {
    def dependencies = deps(a.toList)
    def apply[Z](g: T => Z): Initialize[Z] = new Optional[S, Z](a, g compose f)
    def mapReferenced(g: MapScoped) =
      new Optional(a map mapReferencedT(g).fn, f)
    def validateKeyReferenced(g: ValidateKeyRef) = a match {
      case None => Right(this)
      case Some(i) =>
        Right(new Optional(i.validateKeyReferenced(g).right.toOption, f))
    }
    def mapConstant(g: MapConstant): Initialize[T] =
      new Optional(a map mapConstantT(g).fn, f)
    def evaluate(ss: Settings[Scope]): T =
      f(a.flatMap(i => trapBadRef(evaluateT(ss)(i))))
  ...
  private[sbt] final class Apply[K[L[x]], T](val f: K[Id] => T,
    ...
    def evaluate(ss: Settings[Scope]) =
      f(alist.transform(inputs, evaluateT(ss)))
    def validateKeyReferenced(g: ValidateKeyRef) = {
      ...
      val get = new (ValidatedInit ~> Initialize) {
        def apply[T](vr: ValidatedInit[T]) = vr.right.get
      }
      if (undefs.isEmpty) Right(new Apply(f, alist.transform(tx, get), alist))
      else Left(undefs)
    }
